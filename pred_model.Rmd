...
1. **Feature Ranking**: What player actions or statistics are most predictive of their finish placement?
2. **Clustering**: Which playing styles are most successful?
3. **Model Prediction Accuracy**: How well can we predict a player's finish placement?
...
2. **Passive Strategy**: rather than eliminating other players yourself, the pasive strategy relies on surviving until other players have been eliminated. This typically involves hiding to minimize encounters until only a few players remain, then selectively taking fights until the player is the only one remaining.
...
Additionally, after futher consideration of the data, we realized that does not seem well suited to address the question of strategies for success for the following reasons:
...
Thus, we did explore strategies for success via k-means clustering as k-means clustering always results in $k$ clusters, whether those clusters truly exist or not. 
...
From our analysis of PUBG player data, we have found that 
What characterizes a top performing PUBG player? Can we identify different playstyles for success? And 
