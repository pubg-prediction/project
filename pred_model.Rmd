...
3. Another important element in prediction is the number of players in a game, as indicated by `num_group` and `max_place`. This is likely due to an artifact in sampling in the data. That is, for matches with a lower number of players, it is easier to sample players that place lower, in comparison to matches with a higher number of players. In fact, we can see this phenomenon in the princiapl components plot. As it doesn't have any correspondence to actual playing style, it is of less interest to us. 
...
1. Features can be categorize into groups using PCA: The most important group is kills, followed by number of players. In terms of items, boosts are equally important as weapons. 
