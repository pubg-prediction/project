...
### Linear Regression
First, we fit a linear regression model with 10-fold cross-validation. Since the final win percentage must be a value beteen 0 and 1, we log-transform `1 + win_place_perc` where the `1` is added to ensure all values are defined. The initial model will include all covariates except for the match ID and player ID. These two variables were excluded for two reasons: 

1. The size of the resulting model exhausted the vector memory
2. Both of these variables should not have an impact on the final win percentage of a player as the players in a game are assumed to be randomly selected. Additionally, the player ID does not affect how much data we have on a player as no player appears in more than one game in this data set, so including player ID would be equivalent to adding meaningless dummy variables.
...
varImp(lr_model$finalModel) %>% mutate(variable = rownames(.)) %>%
  arrange(desc(Overall))

# Look at plot of predicted values versus true values on training set
train_solo %>% mutate(predicted = exp(lr_model$finalModel$fitted.values)-1) %>%
...
As we noted earlier in the data exploration, some of the covariates are correlated and some features seem to contain little information or signal (such as distance traveled or swam). Thus, we explore the use of elastic net regression which uses ridge-regression-like penalty to adjust for correlated features and lasso penalty to set the non-informative features to zero. 
...
train_solo %>% mutate(predicted = exp(lasso_fitted)-1) %>%
  ggplot(aes(x = predicted, y = win_place_perc)) +
  geom_point() +
  labs(title = "Predicted Versus Actual Win Place Percentage in the Training Set", 
       x = "Predicted Win Place Percentage", y = "Actual Win Place Percentage") +
...
test_solo %>% mutate(predict = lr_pred) %>%
  ggplot(aes(x = exp(predict)-1, y = win_place_perc)) +
  geom_point() + 
  geom_abline(slope = 1, intercept = 0, color = 'red') +
  labs(title = "Predicted Versus Actual Win Place Percentage in the Test Set", 
       x = "Predicted Win Place Percentage", y = "Actual Win Place Percentage") +
  theme_bw()
...
```{r lasso_test}
# Elastic net
lasso_pred = predict(lasso_model, newdata = test_solo)

# Plot Actual vs. Predicted
test_solo %>% mutate(predict = lasso_pred) %>%
  ggplot(aes(x = exp(predict)-1, y = win_place_perc)) +
  geom_point() + 
  geom_abline(slope = 1, intercept = 0, color = 'red') +
  labs(title = "Predicted Versus Actual Win Place Percentage in the Test Set", 
...
# Get MSE
lasso_MSE = mean((exp(lasso_pred) - 1 - test_solo$win_place_perc)^2)
```
### Summary of Models
