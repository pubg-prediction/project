...
First, we fit a linear regression model with 5-fold cross-validation. Since the final win percentage is a value beteen 0 and 1, we log-transform `1 + win_place_perc` where the `1` is added to ensure all values are defined. However, this still doesn't guarantee that the predicted values will be between 0 and 1, so we constrain the predicted value to be 0 if it is negative and 1 if it is above 1. 
...
To account for correlation between features, another option is to use a random forest model. Due to computational costs, we make the following choices:

1. Train only on 10,000 observations.
2. Use only the top $10$ features determined by elastic net regression.
3. Use 100 trees.

Using cross-validation, we tune hyperparameters for random forest.
...
predictions_solo = predictions_solo %>%
  mutate(rf_pred = predict(rf_model, val_solo),
         rf_pred = case_when(rf_pred < 0 ~ 0,
                             rf_pred > 1 ~ 1,
                             rf_pred >=0 & rf_pred <= 1 ~ rf_pred)) # Constrain values to [0,1]
...
3. Random forest: importance scores, which corresponds to the decrease in out of bag accuracy if the feature is randomly permuted
...
There is not much agreement in the features that each model regards to be important. This may be due to highly correlated features; for example, `kill_place` and `kill_streaks` are highly correlated and while the former is rated as highly important by the linear regression and random forest, the latter is rated as highly important by elastic net regression. We will continue investigating features and features importance in the dimension reduction and clustering section.

## Comparison of Models

We will compare models with the following metrics:

1. Mean absolute error (MAE): Represents the average absolute deviation. $\frac{\sum_{i=1}^n |\hat{y}_i - y_i|}{n}$

2. Self-defined accuracy metric (SDAM): If the predicted outcome is within 5% of the actual win place percentage, we classify it as a "correct" prediction. Otherwise, it is an incorrect prediction. $\frac{\sum_{i=1}^n \mathbb{1}_{|hat{y}_i - y_i| <= 0.05}}{n}$

We will compare the models using these metrics on the validation set.
...
metrics_solo %>%
  ...
  geom_col() + 
  geom_label() + 
  labs(title = "Comparison of SDAM", x = "Model", y = "SDAM") +
  scale_x_discrete(labels = c("Elastic Net", "Linear", "Random Forest")) +
  theme_bw()
```

From the above plots, we can see that random forest performs best on both metrics. This is despite the fact that random forest was trained on only 10,000 observations as opposed to 60,000 observations. Its MAE is 0.054, which means that on average, the predicted win place percentage is 0.057 off from the true win place percentage. Its SDAM is 0.604, which means that for 60.4% of observations in our validation set, the predicted value is within 5% of the true win place percentage.
...
```{r lm_test}
pred_val_test = data.frame(rf_test_pred = predict(rf_model, test_solo)) %>%
...
On the test set, random forest has a MAE of `r round(pred_val_test$rf_test_mae, 3)` and a SDAM of `r round(pred_val_test$rf_test_sdam, 3)`, which is similar to what we saw in the validation set.

## Summary

**Best-performing model: Random forest**

We use three different models to predict the win place percentage: linear regression, elastic net regression, and random forest. Using mean absolute error (MAE) and a similar, self-defined accuracy metric (SDAM) quantifying the proportion of predictions that fall within 5% of the true win place percentage, we found that the random forest model performed best using 5-fold cross validation on the training set. On an independent test set, we achieved a MAE of `r round(pred_val_test$rf_test_mae, 3)` and a SDAM of `r round(pred_val_test$rf_test_sdam, 3)`.

**High predictive accuracy for top players**

With the random forest model in particular, we can also see from its figure of predicted vs actual values that our predictions are more accurate for players with higher placements. This makes sense since there should be increasingly higher demands on skill the higher you place and therefore more signal in the data. In other words, the difference in skill between a player who places 60 vs a player who places 90 out of 100 is likely much smaller than the difference between a player who places 1 vs a player who places 30 out of 100. As a result, we can more accurately distinguish between places the higher you place. 
...
Our outcome feature `win_place_perc` is pretty well explained by the first princiapl component. 
