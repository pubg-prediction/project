...
lr_model = train(log(win_place_perc + 1) ~ . - match_id - id, data = train_solo, 
                 method = "lm", trControl = tc, trace = FALSE)
summary(lr_model)

# Plot predicted values versus true values on training set
predictions_solo = data.frame(win_place_perc = train_solo$win_place_perc,
                              lm_pred = exp(lr_model$finalModel$fitted.values) - 1) %>%
...
```{r}
# Table goes here
lr_MSE = mean((exp(lr_model$finalModel$fitted.values) - 1 - test_solo$win_place_perc)^2)
```


Here we can provide a table (or plot) of MSE (or some metric representing accuracy, caret probably has a default) for each model. Then we say X model has the highest accuracy.
