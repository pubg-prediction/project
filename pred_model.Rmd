...
In the training set, `n_groups` = `max_place` for all matches, which means that we have data for all players up to the `max_place` player. Therefore, we remove the `n_groups` feature.
...
data.frame(PC1 = pc$x[,1], PC2 = pc$x[,2]) %>%
  ggplot(aes(PC1, PC2, color = (winners_solo$n_groups))) +
  geom_point(alpha = 0.5) +
