...
train_solo %>% mutate(predicted = exp(lr_model$finalModel$fitted.values)-1) %>%
  ...
  geom_point() +
  labs(main = "Predicted Versus Actual Win Place Percentage in the Training Set", 
...
```{r rf, eval = F}
rf_model = train(log(win_place_perc+1) ~ . - match_id - id, data = train_solo, method = "rf", trControl = tc)
summary(rf_model)
```
