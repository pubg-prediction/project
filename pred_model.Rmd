...
# Save predictions
predictions_solo = data.frame(win_place_perc = train_solo$win_place_perc,
                              lm_pred = exp(lm_model$finalModel$fitted.values) - 1) %>%
...
predictions_solo = predictions_solo %>%
  mutate(lasso_pred = exp(predict(lasso_model, train_solo)) - 1,
...
To account for correlation between features, another option is to use a random forest model. Due to computational costs, we first select the top $5$ features determined by elastic net regression and only use those features to train the random forest model. Furthermore, we only use 100 trees.
...
rf_model = train(as.formula(paste("win_place_perc ~ ", paste(imp_features, collapse = "+"))), 
                 data = train_solo, method = "rf", ntree = 100, trControl = tc)
print(rf_model)

# Add predictions to df
predictions_solo = predictions_solo %>%
  mutate(rf_pred = predict(rf_model, train_solo),
...
Which features are most predictive? With the generic `varImp` function, we can compare the relative importance of features. For the linear models, `varImp` is equivalent to the absolute value of the t-statistic. 
...
lasso_imp = varImp(lasso_model$finalModel) %>%  # absolute value of t-statistic
  mutate(variable = rownames(.)) %>%
  arrange(desc(Overall))

rf_imp = varImp(rf_model$finalModel) %>%
  mutate(variable = rownames(.)) %>%
  arrange(desc(Overall))

bind_rows(lm_imp, lasso_imp, rf_imp)
```

The top 5 features that are
...
# Boxplots of mean absolute error
predictions_solo %>%
  gather(lm_ae, lasso_ae, rf_ae) %>%
```
...
```{r lm_test}
# Linear regression
pred_test = predict(lm_model, newdata = test_solo)
...
To summarize, we use a couple of different models to predict the win place percentage. With X as our metric, we found that X model performed best using 5-fold cross validation on the training set. On an independent test set, the accuracy of X model was X. 
