...
We are given a training set and a test set. The outcome variable for the test set will not be given to us until the end of the Kaggle competition in Jan. 30th, 2019. Therefore, for the purposes of this project, we will only be using the provided training set. Within the provided training set, we will create our own "training" (80%) and "test" set (20%).
...
In our data set, we have `r n_distinct(train_solo$match_id)` players with no duplicates (i.e. no player has participated in more than one game). Additionally our training set consists of data for `r n_distinct(train_solo$match_id)` matches with data for on average around 80\% of players. 

```{r base_char_data}
# Look at unique players
length(train_solo$id)
n_distinct(train_solo$id)

# Look at unique matches
n_distinct(train_solo$match_id)
...
train_solo %>% group_by(match_id, max_place, match_duration) %>% count() %>%
  mutate(prop_data = n/max_place) 
```

The number of unique player IDs is `r n_distinct(train_solo$id)` and the number of unique matches is `r n_distinct(train_solo$match_id)`.
...
train_solo %>% mutate(win_place_cat = as.factor(floor(win_place_perc / 2 * 10) * 20)) %>%
