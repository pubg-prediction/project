...
First, we fit a linear regression model to get a sense of the predictive accuracy we are able to achieve without tuning any parameters as well as which features are significant (with the caveat that we know there is high correlation between some of the features).

```{r}
# caret method = "lm"
# caret method = "penalized"
```
