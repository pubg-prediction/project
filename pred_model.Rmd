...
2. **Self-defined accuracy metric (SDAM)**: If the predicted outcome is within 5% of the actual win place percentage, we classify it as a "correct" prediction. Otherwise, it is an incorrect prediction. 
$$SDAM = \frac{\sum_{i=1}^n \mathbb{1}_{|\hat{y}_i - y_i| <= 0.05}}{n}$$
...
data.frame(cutoff = rep(cutoff, 3), 
  ...
  geom_point() +
  geom_line() +
  labs(title = "Proportion of Predictions within X% of Actual Win Place Percentage",
...
On the test set, random forest has a MAE of `r round(pred_test_metrics$rf_test_mae, 3)` and a SDAM of `r round(pred_test_metrics$rf_test_sdam, 3)`, which is similar to what we saw in the validation set. We can also see in the above plot that the random forest model performs equally well in the test set as in the validation set.
